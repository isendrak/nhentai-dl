...
namespace NHentaiDL {
	public class PururinUs : ISitePlugin {
		...
		private Regex _URLRegex = new Regex("pururin.us/(gallery/([0-9]+)|read/[0-9]+/[0-9]+)/[0-9a-zA-Z-]+");
		public GalleryInfo GetGallery(string GalleryUrl) {
			...
			if(m.Groups[1].Value.ToLower().IndexOf("read/", StringComparison.InvariantCulture) == -1) {
				GalleryUrl = "http://pururin.us/read/" + m.Groups[2].Value + "/01" + GalleryUrl.Substring(GalleryUrl.LastIndexOf('/'));
			}
			...
			foreach(Match match in Regex.Matches(Body, "http:\\\\/\\\\/pururin\\.us\\\\/assets\\\\/images\\\\/data\\\\/([0-9]+)\\\\/([0-9]+)\\.([a-zA-Z0-9]+)")) {
				Images.Add(new ImageInfo { Filename = string.Format("{0:0000}.{1}", Page, match.Groups[3].Value), URL = new Uri(string.Format("http://www.pururin.us/assets/images/data/{0}/{1}.{2}", match.Groups[1].Value, match.Groups[2].Value, match.Groups[3].Value)) });
				++Page;
			}
