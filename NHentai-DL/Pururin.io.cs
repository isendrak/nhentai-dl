...
namespace NHentaiDL {
	public class PururinIo : ISitePlugin {
		public string Name { get { return "Pururin.io"; } }
		public string ExampleURL { get { return "pururin.io/gallery/12345/gallery-name"; } }
		public Regex URLRegex { get { return _URLRegex; } }
		private Regex _URLRegex = new Regex("pururin.io/(gallery/([0-9]+)|read/[0-9]+/[0-9]+)");
		public GalleryInfo GetGallery(string GalleryUrl) {
			...
			if(m.Groups[1].Value.ToLower().IndexOf("read/", StringComparison.InvariantCulture) == -1) {
				GalleryUrl = "http://pururin.io/read/" + m.Groups[2].Value + "/01";
			}
			string Body = wc.DownloadString(GalleryUrl);
			string Title = Regex.Match(Body, "<div class=\"title\">([^<]+)</div>").Groups[1].Value.Trim();
			int Page = 1;
			foreach(Match match in Regex.Matches(Body, "http:\\\\/\\\\/pururin\\.io\\\\/assets\\\\/images\\\\/data\\\\/([0-9]+)\\\\/([0-9]+)\\.([a-zA-Z0-9]+)")) {
				Images.Add(new ImageInfo { Filename = string.Format("{0:0000}.{1}", Page, match.Groups[3].Value), URL = new Uri(string.Format("http://pururin.io/assets/images/data/{0}/{1}.{2}", match.Groups[1].Value, match.Groups[2].Value, match.Groups[3].Value)) });
				++Page;
			}
